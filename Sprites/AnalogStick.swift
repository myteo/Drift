...
open class AnalogJoystick: SKNode {
    ...
    convenience init(diameters: (substrate: CGFloat, stick: CGFloat?),
                     colors: (substrate: UIColor?, stick: UIColor?)? = nil, images: (substrate: UIImage?, stick: UIImage?)? = nil) {
    ...
    // MARK: - Overrides
    // Begin tracking only when moved not on start
